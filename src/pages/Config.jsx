...
const SAMPLE_FILES = [
    { label: 'sample-accounts.csv', path: '/datasets/sample-accounts.csv' },
    { label: 'sample-transactions.csv', path: '/datasets/sample-transactions.csv' },
];
...
const Config = () => {
    ...
    const [toast, setToast] = useState(null);
    const [adminId] = useState('admin123');
    const [selectedFile, setSelectedFile] = useState(null); // { name, file | samplePath }
    ...
    const handleSampleUpload = async (samplePath, sampleName) => {
        setUploading(true);
        try {
            ...
            showToast('Failed to load sample file', 'error');
            setUploading(false);
        }
    };
    ...
    const handleConfirmUpload = () => {
        ...
        } else if (selectedFile.samplePath) {
            handleSampleUpload(selectedFile.samplePath, selectedFile.name);
        }
    };
            ...
            <div className="max-w-6xl mx-auto px-6 py-10">
                ...
                <div className="mb-8 rounded-2xl border border-white/[0.08] bg-white/[0.02] overflow-hidden">
                    ...
                    <div className="flex border-b border-white/[0.06]">
                        <button
                            onClick={() => { setUploadMode('local'); setSelectedFile(null); }}
                            className={`flex items-center gap-2 px-5 py-3.5 text-[12.5px] font-semibold transition-all border-b-2 -mb-px ${uploadMode === 'local'
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-3.5 text-[12.5px] font-semibold transition-all border-b-2 -mb-px ${uploadMode === 'sample'
                                ? 'border-indigo-400 text-indigo-300 bg-indigo-500/[0.06]'
                                : 'border-transparent text-slate-500 hover:text-slate-300'}`}
                        >
                            <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
                    ...
                    <div className="p-6">
                        {uploadMode === 'local' ? (
                            ...
                            <div
                                ...
                                onDragLeave={() => setDragActive(false)}
                                onDrop={onDrop}
                                onClick={() => !uploading && !selectedFile && fileRef.current?.click()}
                                className={`relative rounded-xl border-2 border-dashed cursor-pointer transition-all duration-200 overflow-hidden
                                    ${selectedFile ? 'border-indigo-400/50 bg-indigo-500/[0.06] cursor-default' :
                                        dragActive ? 'border-indigo-400 bg-indigo-500/[0.08]' :
                                            'border-white/[0.1] hover:border-indigo-500/50 hover:bg-indigo-500/[0.04]'}`}
                            >
                                <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/[0.03] via-transparent to-purple-500/[0.03] pointer-events-none" />
                                <div className="flex flex-col items-center justify-center py-10 px-6 text-center relative z-10">
                                    {uploading ? (
                                        ...
                                        <div className="flex flex-col items-center gap-3 w-full">
                                            ...
                                            <div>
                                                <p className="text-slate-200 font-semibold text-sm">{selectedFile.name}</p>
                                                <p className="text-slate-500 text-xs mt-0.5">Ready to upload</p>
                                            </div>
                                            <div className="flex gap-2 mt-1">
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); handleConfirmUpload(); }}
                                                    className="px-4 py-1.5 rounded-lg bg-indigo-500 hover:bg-indigo-600 text-white text-[12px] font-semibold transition-all"
                                                >
                                                    Upload
                                                </button>
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); setSelectedFile(null); }}
                                                    className="px-4 py-1.5 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] text-slate-300 text-[12px] font-medium transition-all"
                                                >
                                                    Cancel
                                                </button>
                                            </div>
                                        ...
                                        <>
                                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 transition-all ${dragActive ? 'bg-indigo-500/20 scale-110' : 'bg-white/[0.05]'}`}>
                                                <svg width="26" height="26" fill="none" stroke={dragActive ? '#818cf8' : '#64748b'} strokeWidth="1.8" viewBox="0 0 24 24">
                                                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                                                    <polyline points="17 8 12 3 7 8" />
                                                    <line x1="12" y1="3" x2="12" y2="15" />
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {SAMPLE_FILES.map((sf) => {
                                    const isSelected = selectedFile?.name === sf.label;
                                    return (
                                        <button
                                            key={sf.label}
                                            onClick={() => setSelectedFile(isSelected ? null : { name: sf.label, samplePath: sf.path })}
                                        ...
                                        >
                                            <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 transition-all
                                                ${isSelected ? 'bg-indigo-500/20 border border-indigo-500/30' : 'bg-white/[0.05] border border-white/[0.08]'}`}>
                                                <svg width="15" height="15" fill="none" stroke={isSelected ? '#818cf8' : '#64748b'} strokeWidth="1.9" viewBox="0 0 24 24">
                                                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                                                    <polyline points="14 2 14 8 20 8" />
                                                    <line x1="9" y1="13" x2="15" y2="13" />
                                                    <line x1="9" y1="17" x2="15" y2="17" />
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-[13px] font-semibold truncate">{sf.label}</p>
                                                <p className={`text-[11px] mt-0.5 ${isSelected ? 'text-indigo-400' : 'text-slate-500'}`}>Sample dataset</p>
                                            </div>
                                            {isSelected && (
                                ...
                                {selectedFile && (
                                    <div className="sm:col-span-2 flex items-center gap-3 pt-1">
                                        <div className="flex items-center gap-2 flex-1 px-3.5 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.07]">
                                            <svg width="13" height="13" fill="none" stroke="#818cf8" strokeWidth="2" viewBox="0 0 24 24">
                                                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                                                <polyline points="14 2 14 8 20 8" />
                                            </svg>
                                            <span className="text-slate-300 text-[12.5px] font-medium">{selectedFile.name}</span>
                                            <span className="text-slate-500 text-[11px] ml-1">selected</span>
                                        </div>
                                        <button
                                            onClick={handleConfirmUpload}
                                            disabled={uploading}
                                            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white text-[12.5px] font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                                        >
                                            {uploading ? (
                                                <>
