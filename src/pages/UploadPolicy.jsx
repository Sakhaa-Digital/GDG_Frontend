...
/* ─── Sample Policy Files (add more here as needed) ─── */
const SAMPLE_POLICIES = [
    { label: "Anti-Corruption Policy", filename: "Anti.pdf", path: "/policies/Anti.pdf", type: "application/pdf" },
    { label: "Final Policy", filename: "finalpolicy.pdf", path: "/policies/finalpolicy.pdf", type: "application/pdf" },
    // Add more sample files here — just add an entry and drop the file in /policies/
];
...
/* ─── Processing Banner ─── */
function ProcessingBanner({ policyName, onDismiss }) {
    const [elapsed, setElapsed] = useState(0);
    useEffect(() => {
        const t = setInterval(() => setElapsed((e) => e + 1), 1000);
        return () => clearInterval(t);
    }, []);
    const pct = Math.min((elapsed / 150) * 100, 95); // ~2.5 min for 95%

    return (
        <div className="relative overflow-hidden bg-amber-500/[0.07] border border-amber-400/25 rounded-2xl px-5 py-4 flex items-start gap-4">
            {/* animated shimmer */}
            <div className="absolute inset-0 -translate-x-full animate-[shimmer_2.5s_infinite] bg-gradient-to-r from-transparent via-amber-400/[0.06] to-transparent pointer-events-none" />

            <div className="w-9 h-9 rounded-xl bg-amber-400/10 border border-amber-400/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                <svg width="17" height="17" fill="none" stroke="#fbbf24" strokeWidth="2" viewBox="0 0 24 24">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-3 mb-1.5">
                    <p className="text-amber-300 font-semibold text-[13px]">
                        Extracting rules from <span className="text-amber-200">"{policyName}"</span>
                    </p>
                    <button
                        onClick={onDismiss}
                ...
                <p className="text-amber-500/80 text-[11.5px] mb-3">
                    This usually takes 2–3 minutes. The policy will become active once processing is complete.
                </p>
                {/* progress bar */}
                <div className="h-1.5 w-full bg-amber-400/10 rounded-full overflow-hidden">
                    <div
                        className="h-full bg-gradient-to-r from-amber-400 to-amber-300 rounded-full transition-all duration-1000"
                        style={{ width: `${pct}%` }}
                    />
                </div>
                <p className="text-amber-600 text-[10.5px] mt-1.5 font-mono">
                    {elapsed < 60
                        ? `${elapsed}s elapsed`
                        : `${Math.floor(elapsed / 60)}m ${elapsed % 60}s elapsed`}
                    {" · "}approx. {Math.max(0, Math.round((150 - elapsed) / 60))}–{Math.max(1, Math.round((180 - elapsed) / 60))} min remaining
                </p>
            </div>
        </div>
    );
            ...
            <div className="p-4 flex flex-col gap-3">
              ...
              ${policy.active ? "bg-indigo-500" : "bg-slate-700"}`}>
                ...
                ${policy.active ? "translate-x-4" : "translate-x-0"}`} />
                        ...
                        <span className={`text-[11px] font-semibold transition-colors ${policy.active ? "text-indigo-400" : "text-slate-500"}`}>
                            {policy.active ? "Active" : "Inactive"}
                        </span>
                    </button>
...
export default function UploadPolicy() {
    ...
    const [activeTab, setActiveTab] = useState("active");
    const [togglingId, setTogglingId] = useState(null);
    const [uploadMode, setUploadMode] = useState("local"); // 'local' | 'sample'
    const [selectedSample, setSelectedSample] = useState(null);
    const [processingBanners, setProcessingBanners] = useState([]); // [{id, name}]
    ...
    const addProcessingBanner = (name) => {
        const id = Date.now();
        setProcessingBanners((prev) => [...prev, { id, name }]);
    };

    const dismissBanner = (id) => {
        setProcessingBanners((prev) => prev.filter((b) => b.id !== id));
    };

    /* ── upload local file ── */
    const handleUpload = async (fileToUpload, nameOverride) => {
        ...
        const params = new URLSearchParams({ admin_id: adminId });
        const finalName = nameOverride || policyName.trim() || file.name;
        if (finalName) params.append("policyName", finalName);
        try {
            const res = await fetch(`${API_BASE}/policies/?${params}`, { method: "POST", body: formData });
            if (!res.ok) throw new Error();
            toast.success("Policy uploaded! Rules are being extracted…");
            addProcessingBanner(finalName);
            resetModal();
            fetchPolicies();
        } catch {
            toast.error("Upload failed. Please try again.");
    ...
    const handleSampleUpload = async () => {
        ...
        try {
            ...
            const file = new File([blob], selectedSample.filename, { type: selectedSample.type });
            const name = policyName.trim() || selectedSample.label;
            await handleUpload(file, name);
        } catch {
            toast.error("Failed to load sample file. Check the file exists in /policies/");
            setUploading(false);
        }
    };
    ...
    return (
        <div className="flex flex-col min-h-screen bg-slate-950 text-slate-100 p-6 gap-6">
            ...
            {/* ── Processing Banners ── */}
            {processingBanners.length > 0 && (
                <div className="flex flex-col gap-3">
                    {processingBanners.map((b) => (
                        <ProcessingBanner
                            key={b.id}
                            policyName={b.name}
            ...
            {showUploadModal && (
                ...
                >
                    <div className="bg-slate-900 border border-white/[0.09] rounded-2xl shadow-2xl w-full max-w-md">
                        {/* Modal header */}
                        <div className="flex items-center justify-between px-7 pt-6 pb-5 border-b border-white/[0.06]">
                            <div>
                                <h2 className="text-[17px] font-bold text-slate-100">Add Policy</h2>
                                <p className="text-slate-500 text-[12px] mt-0.5">Upload a document or load sample data</p>
                            </div>
                            <button
                                onClick={resetModal}
                                className="w-8 h-8 flex items-center justify-center rounded-lg bg-white/[0.05] hover:bg-white/[0.1] text-slate-400 transition-colors"
                        ...
                        <div className="flex border-b border-white/[0.06]">
                            <button
                                onClick={() => { setUploadMode("local"); setSelectedSample(null); }}
                                className={`flex items-center gap-2 px-5 py-3 text-[12.5px] font-semibold transition-all border-b-2 -mb-px ${uploadMode === "local"
                                    ? "border-indigo-400 text-indigo-300 bg-indigo-500/[0.06]"
                            ...
                            <button
                                onClick={() => { setUploadMode("sample"); setSelectedFile(null); }}
                                className={`flex items-center gap-2 px-5 py-3 text-[12.5px] font-semibold transition-all border-b-2 -mb-px ${uploadMode === "sample"
                                    ? "border-indigo-400 text-indigo-300 bg-indigo-500/[0.06]"
                                    : "border-transparent text-slate-500 hover:text-slate-300"}`}
                            >
                                <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
                                ...
                                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400 font-bold">
                                    {SAMPLE_POLICIES.length}
                                </span>
                            </button>
                        </div>

                        <div className="p-7 flex flex-col gap-5">
                            {/* Policy name input (always shown) */}
                            <div>
                                <label className="block text-[11px] font-semibold uppercase tracking-widest text-slate-500 mb-2">
                                    Policy Name <span className="text-slate-600 normal-case tracking-normal font-normal">(optional)</span>
                                </label>
                                <input
                                    value={policyName}
                                    onChange={(e) => setPolicyName(e.target.value)}
                                    placeholder={
                                        uploadMode === "sample" && selectedSample
                                            ? selectedSample.label
                                            : "e.g. Data Privacy Policy 2025"
                                    }
                                    className="w-full bg-slate-800 border border-white/[0.08] rounded-xl px-4 py-3 text-[13px] text-slate-200 placeholder-slate-600 outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/30 transition-all"
                                />
                            </div>
                            ...
                            {uploadMode === "local" ? (
                                /* ── Local upload ── */
                                selectedFile ? (
                                    <div className="flex items-center gap-3 bg-slate-800 border border-white/[0.08] rounded-xl px-4 py-3">
                                        <FileIcon sourceType={selectedFile.type} size={20} />
                                        <span className="flex-1 text-[13px] text-slate-300 truncate">{selectedFile.name}</span>
                                        <button
                                            onClick={() => setSelectedFile(null)}
                      ...
                      ${dragOver
                                ...
                                <div className="flex flex-col gap-2">
                                    {SAMPLE_POLICIES.map((sp) => {
                                        const isSelected = selectedSample?.filename === sp.filename;
                                        return (
                                            <button
                                                key={sp.filename}
                                                onClick={() => setSelectedSample(isSelected ? null : sp)}
                                                className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all
                          ${isSelected
                                                        ? "border-indigo-400/60 bg-indigo-500/[0.1] text-indigo-200"
                                                        : "border-white/[0.08] bg-white/[0.02] text-slate-300 hover:border-indigo-500/40 hover:bg-indigo-500/[0.05]"}`}
                                            >
                                                <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 transition-all
                            ${isSelected ? "bg-red-500/15 border border-red-500/25" : "bg-white/[0.05] border border-white/[0.08]"}`}>
                                                    <FileIcon sourceType={sp.type} size={15} />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-[13px] font-semibold truncate">{sp.label}</p>
                                                    <p className={`text-[11px] mt-0.5 font-mono ${isSelected ? "text-indigo-400" : "text-slate-500"}`}>
                                                        {sp.filename}
                                                    </p>
                                                </div>
                                                {isSelected && (
                                                    <svg width="16" height="16" fill="none" stroke="#818cf8" strokeWidth="2.5" viewBox="0 0 24 24">
                                    ...
                                    {SAMPLE_POLICIES.length === 0 && (
                                        <p className="text-slate-500 text-[12.5px] text-center py-6">
                                            No sample policies configured. Add entries to <code className="text-indigo-400 text-[11px]">SAMPLE_POLICIES</code> in the component.
                                        </p>
                                    )}
                                </div>
                            )}
                            ...
                            <div className="flex gap-3 pt-1">
                                ...
                                <button
                                    onClick={uploadMode === "local" ? () => handleUpload() : handleSampleUpload}
                                    disabled={uploading || (uploadMode === "local" ? !selectedFile : !selectedSample)}
                                    className="flex-[2] py-2.5 bg-indigo-500 hover:bg-indigo-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-[13px] font-semibold rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/20"
                                >
                                    {uploading ? (
                                        <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Uploading…</>
                                    ) : uploadMode === "sample" ? "Load Sample Policy" : "Upload Policy"}
                                </button>
                            </div>
                        </div>
                    </div>
